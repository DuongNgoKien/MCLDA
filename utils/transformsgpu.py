...
def cutmix(labels_2, s_class, lam = 0.8):
    # Get bounding box
    ctotal = (labels_2 == s_class).sum().item()
    for _ in range(20):
        bbx1, bby1, bbx2, bby2 = get_rand_bbox(labels_2.shape, lam)
        label_pasted = labels_2[bbx1:bbx2, bby1:bby2]
        total = (label_pasted == s_class).sum().item()
        if total >= ctotal*0.5:
            break
