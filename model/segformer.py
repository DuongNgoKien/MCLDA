...
class SegFormerHead(nn.Module):
    def __init__(self, dims: list, embed_dim: int = 256, num_classes: int = 19):
        ...
        for i, dim in enumerate(dims):
            self.add_module(f"linear_c{i+1}", MLP_Decoder(dim, embed_dim))
    ...
    def forward(self, features: Tuple[Tensor, Tensor, Tensor, Tensor]) -> Tensor:
        ...
        seg_feat = self.linear_fuse(torch.cat(outs[::-1], dim=1))
        seg = self.linear_pred(self.dropout(seg_feat))
        return seg_feat, seg
...
class SegFormer(nn.Module):
    def __init__(self, type_model, num_classes) -> None:
        super().__init__()
        self.mit_settings = {
            'B0': [[32, 64, 160, 256], [2, 2, 2, 2]],        # [embed_dims, depths]
            'B1': [[64, 128, 320, 512], [2, 2, 2, 2]],
    ...
    def optim_parameters(self, args):
        return [
                {'params': self.backbone.parameters(), 'lr': args.learning_rate},
                {'params': self.decode_head.parameters(), 'lr': args.learning_rate * 10}
            ]

# import torchinfo
# model = SegFormer("B3", 19)
