...
def main():
    ...
    if optimizer_type == 'SGD':
        ...
        if len(gpus) > 1:
            optimizer = optim.AdamW(model.module.optim_parameters(learning_rate_object),
                        lr=learning_rate, momentum=momentum,weight_decay=weight_decay)
        else:
            optimizer = optim.AdamW(model.optim_parameters(learning_rate_object),
                        lr=learning_rate, weight_decay=weight_decay)
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if i_iter == 3000:
            ...
            for i, (src_input_in, src_label_in) in enumerate(source_trainloader):
                ...
                src_feat_in, src_out_in = ema_model(src_input_in)
                B, N, Hs, Ws = src_feat_in.size()

                # source mask: downsample the ground-truth label
                src_mask_in = F.interpolate(src_label_in.unsqueeze(0).float(), size=(Hs, Ws), mode='nearest').squeeze(0).long()
                src_mask_in = src_mask_in.contiguous().view(B * Hs * Ws, )
        ...
        images, labels, _, s_class = batch
        images = images.cuda()
        labels = labels.cuda().long()
        #images, labels = weakTransform(weak_parameters, data = images, target = labels)

        src_feat, s_src_pred = model(images)
        src_pred = interp(s_src_pred)
        L_l = loss_calc(src_pred, labels) # Cross entropy loss for labeled data
        ...
        if train_unlabeled:
            ...
            for image_i in range(batch_size):
                ...
                if mix_mask == "region":
                    if image_i == 0:
                        ...
                        MixMask0 = MixMask0.unsqueeze(0).cuda()
                        smask0 = (torch.from_numpy(smask0)).unsqueeze(0).long().cuda()
                    else:
                        MixMask1, smask1 = transformsgpu.cutmix(labels[image_i], s_class[image_i])
            ...
            feat_u_s, logits_u_s_mix = model(inputs_u_s)
            logits_u_s = interp(logits_u_s_mix)
        ...
        if i_iter >= 3000:
            ...
            mix_mask0 = (smask0[0]*src_mask[0]+(1-smask0[0])*tgt_mask[0]).unsqueeze(0)
            mix_mask1 = (smask1[0]*src_mask[1]+(1-smask1[0])*tgt_mask[1]).unsqueeze(0)
            mixMask = torch.cat((mix_mask0, mix_mask1), 0).long()

            #mix_pseudo_labels0 = (smask0[0]*src_mask[0]+(1-smask0[0])*tgt_pseudo_labels[0]).unsqueeze(0)
            #mix_pseudo_labels1 = (smask1[0]*src_mask[1]+(1-smask1[0])*tgt_pseudo_labels[1]).unsqueeze(0)
            #tgt_pseudo_labels = tgt_pseudo_labels.contiguous().view(B * Hs * Ws, )
            #mix_pseudo_labels = torch.cat((mix_pseudo_labels0, mix_pseudo_labels1), 0).long()
            #mix_pseudo_labels = mix_pseudo_labels.contiguous().view(B * Hs * Ws, )

            #tgt_weight = unlabeled_weight * torch.ones(B*Hs*Ws).cuda()
            #mix_weight0 = smask0 + (1-smask0)*unlabeled_weight
            #mix_weight1 = smask1 + (1-smask1)*unlabeled_weight
            #mix_weight = torch.cat((mix_weight0, mix_weight1), 0)
            mixMask = mixMask.contiguous().view(B * Hs * Ws, )
            ...
            tgt_feat_ema = tgt_feat_ema.permute(0, 2, 3, 1).contiguous().view(B * Hs * Ws, A)
